...
interface TaskStore {
  ...
  addTask: (title: string, description?: string, status?: 'todo' | 'draft', categories?: string[]) => Promise<Task>
  updateTask: (id: number, updates: Partial<Pick<Task, 'title' | 'description' | 'status' | 'position' | 'due_date' | 'category'>> & { categories?: string[] }) => Promise<void>
...
export const useTaskStore = create<TaskStore>((set, get) => ({
  ...
  addTask: async (title, description, status, categories) => {
    const task = await window.api.task.add(title, description, status, categories)
