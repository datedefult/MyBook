...
interface WorkLogStore {
  ...
  addLog: (content: string, categories?: string[], note?: string) => Promise<WorkLog>
...
export const useWorkLogStore = create<WorkLogStore>((set, get) => ({
  ...
  addLog: async (content: string, categories?: string[], note?: string) => {
    const log = await window.api.worklog.add(content, categories, note)
